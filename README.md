...
- **Telemetry-driven escalation**: Transport byte counts, message rates, and resource pressure feed the DoS Guard Manager's Policy Engine. Crossing configured `dos.*` limits raises puzzle cost or triggers a deny, and missing telemetry defaults to `require_challenge`.
