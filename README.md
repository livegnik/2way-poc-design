...
State is a cryptographically verifiable graph, not mutable rows behind an API. Accepted changes form an append-only, ordered history with clear authorship. Sync trusts verification, not transport, timing, or peers.
...
This repository defines the protocol, architecture, and invariants that make those guarantees hold. It is a design repo, not an SDK and not a demo. The goal is a system that stays predictable under failure, adversarial input, and long time horizons.
...
This repo is the main design set for the proof of concept. Normative specs live under `specs/` and define scope, rules, architecture, object models, security, flows, and acceptance criteria, with PoC goals in [`specs/07-poc`](specs/07-poc). Lower-numbered spec folders carry higher authority. When conflicts appear, record an ADR in [`specs/09-decisions`](specs/09-decisions) so exceptions stay visible. `docs-build/` is a non-normative build pack that must stay aligned with the specs.

| Folder | Focus |
| --- | --- |
| [`specs/00-scope`](specs/00-scope) | Vocabulary, boundary, assumptions |
| [`specs/01-protocol`](specs/01-protocol) | Wire format, object model, rules |
| [`specs/02-architecture`](specs/02-architecture) | Managers and services that enforce the protocol |
| [`specs/03-data`](specs/03-data) | Persistence model and lifecycle |
| [`specs/04-interfaces`](specs/04-interfaces) | APIs and event surfaces |
| [`specs/05-security`](specs/05-security) | Threat framing and structural controls |
| [`specs/06-flows`](specs/06-flows) | Bootstrap, sync, recovery, governance flows |
| [`specs/07-poc`](specs/07-poc) | What the proof of concept must demonstrate |
| [`specs/08-testing`](specs/08-testing) | Test categories, conformance, and regression policy |
| [`specs/09-decisions`](specs/09-decisions) | Architecture Decision Records |
| [`specs/10-appendix`](specs/10-appendix) / [`specs/11-examples`](specs/11-examples) | Reference material |
| [`docs-build`](docs-build) | Build planning and traceability (non-normative) |
...
Applications do not manage storage or trust directly. They define schemas, domain logic, and user interfaces, then operate against a constrained system interface. Apps submit proposed changes and the system validates them. Only changes that satisfy schema rules, ownership, and access control become part of shared state.

At the center is a shared graph: the canonical record of identities, devices, apps, relationships, capabilities, and application objects. Every object has explicit ownership. Every relationship is typed. Each change declares which identity authored it and under which rules it is allowed to exist.

History is append-only and ordered. Once accepted, a change cannot be rewritten or physically removed; visibility suppression happens through Rating objects at read time rather than deletes. Entries reference structure, payload, and ancestry so they can be verified and replayed. Peers exchange signed sequences, then validate them locally. A node accepts only what satisfies its own rules, using the same logic it applies to local changes.
...
2WAY defines a protocol for how state is created, validated, ordered, and shared. It is centered on a shared graph rather than APIs or databases. The [protocol overview](specs/01-protocol/00-protocol-overview.md) provides the high-level structure, with supporting architectural context in the [architecture overview](specs/02-architecture/00-architecture-overview.md) and [data flow lifecycle](specs/02-architecture/04-data-flow-overview.md).

All application state is represented as graph records with formal [object model](specs/01-protocol/02-object-model.md) definitions. The model is small and explicit. Parents anchor identities, devices, apps, and domain objects. Attributes attach typed data to Parents. Edges express relationships, capabilities, and transitions. Ratings capture judgments such as endorsements or trust signals. Identifiers and namespaces remain unambiguous across apps and devices, following [Identifiers and namespaces](specs/01-protocol/01-identifiers-and-namespaces.md). Access control is not external logic. It is encoded into the same graph using constrained Parents and Attributes, keeping authorization visible, replayable, and verifiable like any other data, as described in the [access control model](specs/01-protocol/06-access-control-model.md).

Apps are first-class participants in the protocol. Each app has its own namespace, schemas, and portion of the graph. Schemas are defined by apps and stored in the graph itself through the [Schema Manager](specs/02-architecture/managers/05-schema-manager.md) and the shared [object model](specs/01-protocol/02-object-model.md). They specify which object types exist, how they relate, and what values are valid. The protocol does not interpret application meaning. It enforces that meaning structurally and consistently, so every node evaluates the same rules when deciding whether a change is acceptable.

Every proposed change is wrapped in a protocol envelope that specifies the operation records and required identifiers, as defined in [Serialization and envelopes](specs/01-protocol/03-serialization-and-envelopes.md). Sync packages add sender identity, domain metadata, and sequence fields for ordering and replay checks. When a node receives an envelope, it validates it locally. Structure is checked first, including object model invariants and app namespace boundaries. Schema rules are then applied to ensure the change fits the app's schema. Access control is evaluated next using graph-encoded permissions. Only if all checks succeed is the change assigned a global order and committed, with failures classified per [Errors and failure modes](specs/01-protocol/10-errors-and-failure-modes.md). Canonical error shapes and transport mappings are defined in [04-error-model.md](specs/04-interfaces/04-error-model.md). This pipeline runs across the [Graph Manager](specs/02-architecture/managers/07-graph-manager.md), [Schema Manager](specs/02-architecture/managers/05-schema-manager.md), and [ACL Manager](specs/02-architecture/managers/06-acl-manager.md).

History is append-only and ordered per [Sync and consistency](specs/01-protocol/07-sync-and-consistency.md). Nodes exchange signed, ordered envelopes rather than mutable state. Each node replays those sequences independently and accepts only the parts that satisfy its local rules. Ordering comes from protocol-defined sequence assignment, not from message arrival or wall-clock time, with storage-level constraints captured in [Sequences and ordering](specs/03-data/07-sequences-and-ordering.md). This allows nodes to diverge temporarily, operate offline, and still converge without trusting transport or peers.

For the proof of concept, the graph is persisted in SQLite as a relational store, with durability and layout owned by the [Storage Manager](specs/02-architecture/managers/02-storage-manager.md) and detailed in the [data model overview](specs/03-data/00-data-overview.md) and [SQLite layout](specs/03-data/01-sqlite-layout.md). SQLite keeps the PoC storage footprint small and predictable, which makes it realistic to run the same graph persistence layer on desktops, servers, and even embedded devices in theory while still supporting transactional ordering and replay. The protocol remains database-agnostic: it assumes only a storage layer that can persist ordered graph state and replay it deterministically, not a specific engine.
...
The backend makes the rules real. It is one integrated system that decides what is valid, what is allowed, what order changes land in, and what gets stored. If something fails validation, it never becomes part of state, no matter who sent it or how it arrived. This is the opposite of systems that rely on server-side discretion or relay behavior; authority lives in the enforcement pipeline that every node runs locally.
...
The backend is built from singleton managers with narrow jobs that form one path for reads and writes, as defined in the [Managers overview](specs/02-architecture/managers/00-managers-overview.md). The [Config Manager](specs/02-architecture/managers/01-config-manager.md) loads configuration and rejects unsafe settings. The [Auth Manager](specs/02-architecture/managers/04-auth-manager.md) resolves local sessions into an `OperationContext` per [Operation Context](specs/02-architecture/services-and-apps/05-operation-context.md). The [Key Manager](specs/02-architecture/managers/03-key-manager.md) owns private keys and performs signing and decryption for trusted callers. The [App Manager](specs/02-architecture/managers/08-app-manager.md) registers apps and app identities. The [Schema Manager](specs/02-architecture/managers/05-schema-manager.md) checks that a change matches the app's declared structure. The [ACL Manager](specs/02-architecture/managers/06-acl-manager.md) checks that the caller has the right to perform it, using the current `OperationContext`. The [Graph Manager](specs/02-architecture/managers/07-graph-manager.md) is the only place where writes are accepted. It validates object model invariants, assigns a `global_seq`, and commits through the [Storage Manager](specs/02-architecture/managers/02-storage-manager.md). The [State Manager](specs/02-architecture/managers/09-state-manager.md) coordinates ordered sync and package construction from accepted history. The [Network Manager](specs/02-architecture/managers/10-network-manager.md) admits peers, verifies packages using public keys, encrypts payloads with recipient public keys when required, decrypts via the [Key Manager](specs/02-architecture/managers/03-key-manager.md) when required, and never changes protocol data. The [Event Manager](specs/02-architecture/managers/11-event-manager.md) and [Log Manager](specs/02-architecture/managers/12-log-manager.md) record what happened and why. The [Health Manager](specs/02-architecture/managers/13-health-manager.md) and [DoS Guard Manager](specs/02-architecture/managers/14-dos-guard-manager.md) keep the node stable and reject load before correctness can be threatened.

Services sit above the managers and provide higher-level behavior. They turn user intent into protocol-compliant operations, expose backend endpoints, and do app-specific aggregation or validation when needed. They do not get special authority. They cannot write around the managers, bypass schema or ACL checks, or touch keys, storage, or sockets directly. They must always call into the same enforcement path with a complete `OperationContext` as defined in [Operation Context](specs/02-architecture/services-and-apps/05-operation-context.md).

There are two service classes. System services are always present and cover shared workflows that multiple apps rely on. App services are optional and scoped to one app. They exist for backend-heavy features like indexing or specialized queries, and removing them must not break the core or corrupt state.
...
2WAY assumes the network is adversarial and peers can be careless, compromised, or hostile. Security is enforced by protocol structure rather than operator policy. Each device, user, and app holds its own keys and history, and every node verifies incoming changes locally before accepting them per [Cryptography](specs/01-protocol/04-cryptography.md) and [Keys and identity](specs/01-protocol/05-keys-and-identity.md).

All writes, local or remote, use the same envelope path and validation order: structural checks first, then schema rules, then ACL authorization as defined in [Serialization and envelopes](specs/01-protocol/03-serialization-and-envelopes.md) and the [access control model](specs/01-protocol/06-access-control-model.md). Ordering is assigned locally and is repeatable, so replayed or reordered input cannot change outcomes per [Sync and consistency](specs/01-protocol/07-sync-and-consistency.md).

Sybil resistance comes from bounded reach, not global reputation. Identities are key-bound, app namespaces are isolated, and permissions are explicit in the graph, so unknown identities cannot gain broad access without explicit edges or ACL grants per [Identifiers and namespaces](specs/01-protocol/01-identifiers-and-namespaces.md) and the [access control model](specs/01-protocol/06-access-control-model.md). This limits the blast radius of fake identities and impersonation attempts.

Denial-of-service protection is part of the protocol pipeline, not a bolt-on. The DoS Guard Manager gates admission at the network boundary and can require client puzzles before any payload flows inward, following [DoS guard and client puzzles](specs/01-protocol/09-dos-guard-and-client-puzzles.md) and [Network transport requirements](specs/01-protocol/08-network-transport-requirements.md). Puzzle difficulty adjusts dynamically to load and abuse signals, and the system fails closed on policy or puzzle failures. Earlier stages reject malformed input without expensive work, and failures are classified consistently by the [protocol overview](specs/01-protocol/00-protocol-overview.md) and [Errors and failure modes](specs/01-protocol/10-errors-and-failure-modes.md).

Recovery is intentionally simple. Accepted changes are ordered and append-only, and sync packages are signed, so a node can rebuild by replaying history and reapplying the same rules. Unverifiable history is rejected rather than patched or trusted per [Sync and consistency](specs/01-protocol/07-sync-and-consistency.md).
...
Because every device, user, and app keeps its own keys and history, there is no central operator that can extract rent by default. Running a node does not grant control over others, and there is no privileged position to monetize purely by being in the middle.
...
For operators and integrators, the incentive is leverage. By building on 2WAY, they avoid re-implementing identity, permissions, sync, and audit for every system. They gain durability and user trust without having to promise perpetual uptime or benevolent control. Because apps share a common graph model and enforcement rules, they remain interoperable: developers can build on each other's schemas and data without lock-in, and removing one app does not break the system or invalidate the underlying state. 

2WAY can also sit alongside existing systems, integrating at the edges while letting legacy apps and new 2WAY-native apps reinforce each other rather than compete for control.

For users, the incentive is autonomy. Their data, history, and authority persist even if an app, vendor, or service disappears. That means you can move between tools without rewriting your identity or losing the context attached to your records. If a front end goes away, your state is still readable and verifiable because it lives in the shared graph, not in that app's private database. You can keep working offline and sync later without having to trust a central operator to arbitrate what is real. Permissions follow the same rules everywhere, so access does not become a negotiation with each new product. Your history remains auditable, so you can trace who did what and when without relying on screenshots or exports. If you want a different interface, you can adopt another app that speaks the same protocol and continue from the same state. The system favors continuity and user control over vendor-specific gatekeeping. Autonomy here is practical: it protects long-term access, reduces lock-in, and lets users choose tools based on fit rather than fear of losing their data.
...
Apps model their domain data with the same graph primitives: Parents (core entities), Attributes (typed fields), Edges (relations), and Ratings (judgments). That makes relationship queries first-class. Degrees-of-separation queries become bounded graph traversals across explicit edges (contacts-of-contacts, delegated access, trust paths), filtered by schema and ACL so results only include readable nodes. App services can build indexes or materialized views for large datasets, but they still evaluate the same rules and cannot manufacture authority.
