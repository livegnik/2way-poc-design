...
* Defining the `network.*` configuration surface and mandatory limit keys used by [Config Manager](../../../../02-architecture/managers/01-config-manager.md).
* Providing staged admission via a Bastion Engine that isolates unauthenticated peers and coordinates with [DoS Guard Manager](../../../../02-architecture/managers/14-dos-guard-manager.md) for allow, deny, and challenge flows per [01-protocol/09-dos-guard-and-client-puzzles.md](../../../../01-protocol/09-dos-guard-and-client-puzzles.md).
* Executing challenge transport as an opaque exchange, without interpreting puzzle content, difficulty, or verification logic, which are owned by [DoS Guard Manager](../../../../02-architecture/managers/14-dos-guard-manager.md) and defined in [01-protocol/09-dos-guard-and-client-puzzles.md](../../../../01-protocol/09-dos-guard-and-client-puzzles.md).
* Performing peer discovery for first-degree peers, including:
  * requesting peer identity and node endpoint attributes from the [State Manager](../../../../02-architecture/managers/09-state-manager.md)
...
* Binding transport sessions to cryptographic identity only after signature verification (with decryption via [Key Manager](../../../../02-architecture/managers/03-key-manager.md) when required), never through transport-provided identifiers, preserving the identity model in [01-protocol/05-keys-and-identity.md](../../../../01-protocol/05-keys-and-identity.md).
...
* Signing outbound envelopes via the [Key Manager](../../../../02-architecture/managers/03-key-manager.md) and encrypting outbound envelopes using recipient public keys when required by protocol rules and configuration, following the algorithms defined in [01-protocol/04-cryptography.md](../../../../01-protocol/04-cryptography.md).
...
* Transmitting outbound packages received from the [State Manager](../../../../02-architecture/managers/09-state-manager.md), without adding retry semantics or persistence, thus honoring the best-effort semantics of [01-protocol/08-network-transport-requirements.md](../../../../01-protocol/08-network-transport-requirements.md).
* Emitting explicit connection lifecycle events, discovery and reachability events, admission outcomes, transport failures, and health signals to the [Event Manager](../../../../02-architecture/managers/11-event-manager.md) and [Health Manager](../../../../02-architecture/managers/13-health-manager.md), and emitting admission telemetry to [DoS Guard Manager](../../../../02-architecture/managers/14-dos-guard-manager.md) as required by [01-protocol/08-network-transport-requirements.md](../../../../01-protocol/08-network-transport-requirements.md) and [01-protocol/09-dos-guard-and-client-puzzles.md](../../../../01-protocol/09-dos-guard-and-client-puzzles.md).
...
* Surfacing network reachability facts to the [State Manager](../../../../02-architecture/managers/09-state-manager.md) and [Event Manager](../../../../02-architecture/managers/11-event-manager.md) without performing graph writes.
