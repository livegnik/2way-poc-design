...
* Owning all publication surfaces for backend events and enforcing the invariant that all state change notifications flow through [Event Manager](../../../../02-architecture/managers/11-event-manager.md), preserving the single ordered write and notification path mandated in [01-protocol/07-sync-and-consistency.md](../../../../01-protocol/07-sync-and-consistency.md). 
* Accepting normalized event descriptors from [Graph Manager](../../../../02-architecture/managers/07-graph-manager.md), [App Manager](../../../../02-architecture/managers/08-app-manager.md), [Config Manager](../../../../02-architecture/managers/01-config-manager.md), [Network Manager](../../../../02-architecture/managers/10-network-manager.md), [Health Manager](../../../../02-architecture/managers/13-health-manager.md), [DoS Guard Manager](../../../../02-architecture/managers/14-dos-guard-manager.md), and [Log Manager](../../../../02-architecture/managers/12-log-manager.md) after those managers complete their own validation and commit phases so sequencing and trust rules from [01-protocol/00-protocol-overview.md](../../../../01-protocol/00-protocol-overview.md) remain intact. 
* Binding event metadata to [OperationContext](../../../../02-architecture/services-and-apps/05-operation-context.md) derived visibility rules so subscribers can only observe events they are authorized to observe under the access control model described in [01-protocol/06-access-control-model.md](../../../../01-protocol/06-access-control-model.md) and enforced using the identity guarantees in [01-protocol/05-keys-and-identity.md](../../../../01-protocol/05-keys-and-identity.md). 
...
* Emitting audit signals to [Log Manager](../../../../02-architecture/managers/12-log-manager.md) when subscriptions change state, buffers overflow, delivery is suppressed, or component health transitions, matching the failure taxonomy defined in [01-protocol/10-errors-and-failure-modes.md](../../../../01-protocol/10-errors-and-failure-modes.md). 
...
* Defining telemetry control keys (`event.telemetry.heartbeat_interval_ms`, `event.telemetry.degraded_threshold_ms`) used for pipeline health signaling.
* Mapping close reasons and delivery failures to ErrorDetail codes defined by the events interface.
* Reporting readiness, liveness, and queue depth to [Health Manager](../../../../02-architecture/managers/13-health-manager.md), and forwarding repeated abuse signals to [DoS Guard Manager](../../../../02-architecture/managers/14-dos-guard-manager.md), so event surfaces participate in fail closed behavior alongside [01-protocol/09-dos-guard-and-client-puzzles.md](../../../../01-protocol/09-dos-guard-and-client-puzzles.md) directives. 
...
* Schema validation, ACL evaluation of graph writes, graph mutation sequencing, or sync reconciliation. These remain owned by [Schema Manager](../../../../02-architecture/managers/05-schema-manager.md), [ACL Manager](../../../../02-architecture/managers/06-acl-manager.md), [Graph Manager](../../../../02-architecture/managers/07-graph-manager.md), and [State Manager](../../../../02-architecture/managers/09-state-manager.md).
...
* Persistence of historical events, durable replay logs, offline delivery, or guaranteed delivery semantics. [Event Manager](../../../../02-architecture/managers/11-event-manager.md) delivers best effort realtime notifications only. 
* Remote sync propagation or peer to peer message routing. [State Manager](../../../../02-architecture/managers/09-state-manager.md) and [Network Manager](../../../../02-architecture/managers/10-network-manager.md) own remote ingress and egress.
