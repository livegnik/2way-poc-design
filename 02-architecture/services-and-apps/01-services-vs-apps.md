...
For the meta specifications, see [01-services-vs-apps meta](../../10-appendix/meta/02-architecture/services-and-apps/01-services-vs-apps-meta.md).

## 1. Terminology and classification

* **Service** : A backend module that executes inside the node process, translates high level intent into manager calls, and never owns protocol invariants. Two classes exist: system services and app services.
* **System service** : A mandatory service that ships with every node. System services implement shared functionality (bootstrap, provisioning, identity, sync helpers, administrative APIs, etc.). They have no owner app and cannot be removed by app uninstallations.
* **App service** : An optional backend service bound to exactly one registered application slug and `app_id`. These services only exist if an application declares backend code. They must be unloadable without affecting other applications or the core platform.
* **Application (app)** : A namespace and identity registered through App Manager. Every app owns schemas, ACL policy, and UI/UX semantics within its domain. An app may have zero or one app service plus any number of frontend experiences.
...
| Dimension | System service | App service | Application / Frontend |
| --- | --- | --- | --- |
...
2. Apps define namespaces, ACL policy, schemas, and UI semantics. A backend service may exist without an app (system service) or be tethered to exactly one app (app service), but an app may also live without any backend code (frontend only experience).
3. The separation is structural: services run in process but are still treated as untrusted by managers. Apps, including app services, never get privileged access to storage, crypto keys, or network sockets. Violations fail closed and are observable.
...
System services implement functionality that is required for every conforming node, such as account provisioning, identity graph helpers, sync planners, or administrative APIs. Architectural requirements include:
...
App services extend backend behavior for a specific application. Requirements:

* **Binding** : Each app service is bound to a slug/app_id pair resolved by App Manager. The binding is immutable for the lifetime of the node.
* **Loading** : App Manager is the only component allowed to instantiate app services. Modules declare metadata (slug, version, capabilities, minimum manager set) and App Manager wires them after verifying registration and dependencies.
* **Isolation** : App services run within their app's domain. All OperationContext instances created by app services must name the owning `app_id`. They cannot use other app identifiers unless ACL policy explicitly grants cross-app rights and Graph Manager enforces them.
* **Removability** : Uninstalling an app unloads its app service. App services must tolerate being stopped between requests and may not hold global locks that block the backend.
* **Fault containment** : App Manager must be able to disable a faulty app service while leaving the rest of the backend operational. App services must therefore expose a stop hook and avoid shared memory outside their module.
* **Capability limits** : App services cannot: register system services, claim system app identifiers, call other app services directly, or declare their own trust boundaries.
...
### 4.2 App service binding

If an application declares backend logic, it ships an app service module that App Manager wires into the backend. Implementation rules:

* App service modules declare dependency metadata, including manager APIs they require. Missing dependencies are a fatal wiring error.
* App service modules must be versioned. Upgrades require compatibility shims or explicit data migration steps executed through Graph Manager.
...
Frontend apps (native, mobile, CLI, or web) consume backend APIs or sync flows defined by system or app services. Requirements:
...
### 5.2 App service initiated work

App services may run scheduled work (e.g., a digest generator). Requirements:
...
* State Manager validates sync metadata, then hands OperationContext construction off to the receiving service or app service.
...
1. **App identity presence** : Every OperationContext must include `app_id`. System services use `app_0`. App services use their owning `app_id`. Frontend requests must supply the app identifier before any other processing.
...
3. **Capability stamping** : OperationContext includes the specific capability or verb the service is attempting (e.g., `capability=app.messaging.write`). ACL Manager evaluates capabilities alongside graph relationships to determine access.
...
2. App Manager loads application registry, binds identities, and prepares app service metadata.
...
4. App services load only after their owning application schema passes validation. Any failure aborts backend startup or results in the app service being skipped with an audit log entry.
...
* App services must flush derived caches and persist checkpoint metadata via Storage Manager if they need fast resume.
* App Manager marks app services as stopped before managers shut down so no background job attempts to run without manager support.
...
* Removing an app unloads its app service, revokes scheduled jobs, and leaves graph data intact. No other app or service may reuse the freed `app_id`.
...
* App services have no implicit elevation. They must pass through the same ACL and schema gates as system services.
...
7. **Lifecycle hooks** : Start/stop hooks exist, app services can be unloaded cleanly, and system services cooperate with rolling upgrades.
...
9. **Removability** : App services can be removed without affecting other applications, leaving graph data intact for archival or reinstall purposes.
