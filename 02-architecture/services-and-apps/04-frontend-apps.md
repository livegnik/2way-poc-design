...
* `02-architecture/services-and-apps/03-app-backend-extensions.md`
* `02-architecture/services-and-apps/05-operation-context.md`
* `02-architecture/managers/00-managers-overview.md`
* `02-architecture/managers/14-dos-guard-manager.md`
* `04-interfaces/**`
* `05-security/**`
* `06-flows/**`
...
* Describing how frontend apps assemble OperationContext inputs, authenticate users and devices, request capabilities, and respect ACL policy.
...
* Backend manager or service implementation details (`02-architecture/managers/**`, `02-architecture/services-and-apps/02-system-services.md`, `02-architecture/services-and-apps/03-app-backend-extensions.md`).
* Interface definitions already captured in `04-interfaces/**`, except where this document must declare missing shapes to keep frontend development unblocked.
...
* Frontend apps are untrusted clients. Every request they emit must tolerate backend rejection and must never assume privileged access beyond published capabilities (`01-protocol/00-protocol-overview.md`).
* OperationContext inputs (user identity, device identity, app_id, capability intent, trust posture, correlation metadata) are collected and transmitted exactly as defined in `02-architecture/services-and-apps/05-operation-context.md`. Frontends never fabricate server-side context.
* All backend interactions traverse the published HTTP/WebSocket/sync surfaces in `04-interfaces/**` and inherit the authentication, ACL, schema, and ordering rules codified in `01-protocol/03-serialization-and-envelopes.md`, `01-protocol/06-access-control-model.md`, `01-protocol/07-sync-and-consistency.md`, and `01-protocol/08-network-transport-requirements.md`. DoS Guard enforcement and puzzle handling remain entirely on the backend. No direct database, filesystem, or socket manipulation occurs.
* Inputs are validated locally before being sent so malformed payloads never reach the backend, preventing resource abuse and ensuring deterministic error handling per `01-protocol/09-errors-and-failure-modes.md`.
* Frontend storage is non-authoritative. Only graph commits acknowledged by the backend count as truth, cached state can be dropped and regenerated at any time, matching the graph authority rules in `01-protocol/02-object-model.md`.
...
* Sensitive material (keys, tokens, PII) is handled according to `01-protocol/04-cryptography.md` and `05-security/**` guidance, encrypted at rest when stored locally, never logged in plaintext, and scoped to the owning app.
...
* **Runtime context**: The execution container for a surface (browser tab, native view controller, background worker, scheduled automation). Each context must enforce the same OperationContext and security rules.
...
| Client orchestration | Managing sessions, tokens, request queues, retries, background sync, offline caches                                                       | OperationContext fields, capability map, sync manifests            |
| Transport adapters   | Translating orchestration intents into HTTP/WebSocket/sync calls defined in `04-interfaces/**`, encrypting payloads, verifying signatures | Auth credentials, request metadata                                 |
...
| OperationContext Builder | Assembles immutable request metadata: `user_id`, `device_id`, `app_id`, capability verb, correlation ID, locale, client version. |
...
| Sync Controller          | Interfaces with `01-protocol/07-sync-and-consistency.md` flows, schedules pulls and pushes, and validates package signatures.               |
...
## 3. Identity, registration, and OperationContext

### 3.1 Application identity

* Every frontend build is registered with App Manager, resulting in a slug and permanent `app_id` as defined in `01-protocol/01-identifiers-and-namespaces.md`.
...
* Device enrollment flows follow `06-flows/bootstrap/**` guidance and the identity binding guarantees in `01-protocol/05-keys-and-identity.md`. Frontends must store device credentials in secure enclaves or OS-provided key stores.
* Each session binds `user_id`, `device_id`, and `app_id`. Swapping any element requires reconstructing OperationContext from scratch, session reuse across devices is forbidden.
* Device fingerprints, attestation proofs, and hardware posture data must be collected before the first privileged call so Auth Manager can bind them to OperationContext, satisfying the identity requirements described in `01-protocol/05-keys-and-identity.md`.

### 3.3 OperationContext contribution
...
OperationContext objects are immutable snapshots. Once transmitted, the frontend must not reuse or mutate them for retries, new retries reconstruct the context to guarantee deterministic tracing, matching the ordering posture in `01-protocol/03-serialization-and-envelopes.md`.

### 3.4 Capability negotiation

* At authentication, the backend returns capability summaries scoped to the requesting identity and app. Frontends cache them but must re-fetch on ACL changes, session refresh, or `ERR_CAPABILITY_REVOKED` responses (`01-protocol/06-access-control-model.md`).
...
* UI must hide or disable controls for missing capabilities but still allow users to discover the required permissions by referencing documentation provided by Identity Service or App Manager.

## 4. Interface consumption

### 4.1 HTTP and WebSocket usage

* APIs are defined in `04-interfaces/http/**` and `04-interfaces/websocket/**`. Frontends must import generated clients or schemas from those specs rather than re-deriving shapes.
* Requests include:

  * Auth envelope or session token issued by Auth Manager per `01-protocol/03-serialization-and-envelopes.md`.
  * OperationContext payload serialized according to `01-protocol/03-serialization-and-envelopes.md`.
* Responses must be parsed using the canonical error catalog in `01-protocol/09-errors-and-failure-modes.md`. Unknown fields must not crash the client, they must be ignored while preserving the signed body for optional logging.
* WebSocket clients must implement backpressure aligned with `01-protocol/08-network-transport-requirements.md`, if the backend signals `slow_client`, the frontend pauses subscriptions until it drains local queues.

### 4.2 Sync packages

* Sync flows follow `01-protocol/07-sync-and-consistency.md`. Frontends request packages by domain and range, validate signatures, ensure schema compatibility, and apply only after local policy approves.
* Pending mutations are queued in deterministic order, signed locally, and submitted when connectivity allows. Each pending mutation references the schema ID and parent objects used so Graph Manager can verify structure without contacting the frontend again, honoring `01-protocol/02-object-model.md` and `01-protocol/03-serialization-and-envelopes.md`.
...
* Push notifications or background fetches must be relayed through the backend interface layer. Direct peer connections initiated by the frontend are forbidden by the transport boundaries in `01-protocol/08-network-transport-requirements.md`.
* Background jobs advertise their cadence, maximum run time, and expected OperationContext capability to the backend via registration endpoints so backend schedulers can throttle or pause them as needed.
...
* Uploads use signed URLs or chunked flows defined in `04-interfaces/http/upload.md`. Clients must hash chunks, include digests, and resume interrupted uploads without restarting entire files.
...
* Authentication flows (password, passkey, device token) are documented in `04-interfaces/auth/**` and must produce the identity proofs described in `01-protocol/05-keys-and-identity.md`. Clients must implement multi-factor prompts when required by policy.
...
* TLS or Noise transports defined in `01-protocol/08-network-transport-requirements.md` are mandatory. Clients must validate certificates or static keys pinned by the deployment.
...
* Crash dumps and telemetry scrub PII unless the user opts into diagnostic uploads vetted by `05-security/**`.
...
* Local caches must conform to the schemas defined by Schema Manager and the object grammar in `01-protocol/02-object-model.md`. Each cached object carries `schema_version` and `app_id`. When Schema Manager reports a new version, clients invalidate incompatible cached entries.
...
* All outbound mutations live in a write-ahead log storing: payload, schema ID, parent selectors, OperationContext snapshot, submission timestamp, attempt counter, and status so they can be replayed deterministically per `01-protocol/03-serialization-and-envelopes.md`.
* The log enforces FIFO ordering within each parent object unless the interface contract specifies idempotent commutation, satisfying the ordering expectations in `01-protocol/03-serialization-and-envelopes.md`.
...
* Secrets (session tokens, private keys, capability grants) remain in platform key stores. The app retrieves them only when signing or authenticating requests per `01-protocol/04-cryptography.md`, zeroing memory afterward.
...
3. When connectivity returns, the request queue replays pending writes in deterministic order, re-signing with fresh timestamps and OperationContext snapshots.
4. If schema or ACL changes render a pending write invalid, the client prompts the user with context and recommended actions, honoring `01-protocol/02-object-model.md` and `01-protocol/06-access-control-model.md`.
...
* When the backend rejects a mutation with `ERR_OBJECT_VERSION` or equivalent (`01-protocol/09-errors-and-failure-modes.md`), the client fetches the latest object, highlights conflicting fields, and allows the user to reapply or discard changes.
...
Clients must map backend error codes (from `01-protocol/09-errors-and-failure-modes.md`) to UI states:
...
* Retries follow per-endpoint budgets defined in `04-interfaces/**`. Clients respect `Retry-After` headers and any backend-provided admission window.
...
* Logs include timestamp, severity, correlation ID, OperationContext summary (redacted), request and response identifiers, and error codes so backend auditors can map them to the canonical error catalog in `01-protocol/09-errors-and-failure-modes.md`.
...
* When backend surfaces support it, clients publish aggregated metrics via `POST /system/ops/clients/telemetry` (to be documented in `04-interfaces/http/ops.md`).
...
* Builds embed manifest data: slug, `app_id`, semantic version, supported platforms, minimum backend version, capability catalog checksum, and commit hash so compatibility checks align with `01-protocol/10-versioning-and-compatibility.md`.
...
* Clients check for updates via App Manager-provided endpoints, applying the negotiation rules in `01-protocol/10-versioning-and-compatibility.md`. Autoupdate policies respect user settings and enterprise controls.
...
* Installation registers the device with Device Manager via bootstrap flows (`01-protocol/05-keys-and-identity.md`).
...
* Frontend threat assumptions mirror `05-security/**` and the crypto/posture controls in `01-protocol/04-cryptography.md`, the device may be compromised, but the app must minimize secrets at rest, detect tampering where possible, and fail safely when invariants break.
...
* Dependencies are pinned with checksums, verified at build time, and scanned per `05-security/dependency-policy.md`.
...
* Exports occur via backend-approved flows (for example, zipped graph slices). Clients never bypass the backend to read raw storage, preserving the ordering and authorship guarantees in `01-protocol/03-serialization-and-envelopes.md`.
* When offering offline exports, clients encrypt archives and require user-provided passphrases, following the key-handling rules in `01-protocol/04-cryptography.md`. Keys are never reused.

## 12. Backend integration semantics

### 12.1 System service interactions

* Frontends must obey service-specific requirements from `02-architecture/services-and-apps/02-system-services.md`. For example:
...
* When an app extension exposes new APIs, the frontend must fetch the manifest (version, capability list, schema requirements) from App Manager before calling them.
...
### 12.3 OperationContext verification

* Frontends log the OperationContext hash the backend echoes in responses to prove end-to-end traceability, preserving the envelope commitments in `01-protocol/03-serialization-and-envelopes.md`. Hash mismatches trigger alerts and block further operations until resolved.
* For long-running workflows (multi-step forms), the client stores interim OperationContext templates to ensure the final submission matches the initial capability intent.

## 13. Application configuration and build lifecycle

### 13.1 Configuration contract

* App configuration lives in graph-backed objects owned by the app and mirrored in Config Manager under `app.<slug>.*`, following the ownership rules in `01-protocol/02-object-model.md`. Frontends must expose UI or CLI to edit documented keys, validate input locally, and submit mutations via the same manager pipeline system services use (schema -> ACL -> graph).
* Each configuration key declares: datatype, validation rules, required capability for mutation, default value, and rollout semantics (static, hot-reloadable, experiment gated). Documentation belongs inside the app manifest so App Manager can expose it to operators.
* Configuration snapshots are versioned. Clients include the current config hash in OperationContext metadata so backend services can reject stale assumptions with `ERR_CONFIG_STALE`.
* Sensitive configuration (API secrets, federation tokens) never travels through plaintext UI flows. Instead, clients request pre-signed upload slots that encrypt the value client side per `01-protocol/04-cryptography.md`, produce an object reference, and store only handles in configuration entries.

### 13.2 Build and packaging workflow

* Build pipelines ingest schemas, capability catalogs, and configuration defaults from the graph, generate strongly typed clients from `04-interfaces/**`, and embed immutable manifests plus compatibility metadata directly into artifacts.
...
* Continuous integration executes lint, unit, schema validation, and OperationContext assembly tests on every change. Builds fail if generated interface clients differ from checked-in versions, preventing drift from backend contracts.
* Release automation signs artifacts (per `01-protocol/04-cryptography.md`), pushes them to distribution channels, and publishes manifest updates to App Manager so nodes can verify compatibility before accepting the new build.
...
* Environment-specific configuration (API endpoints, telemetry sinks, feature flags) is injected via signed config bundles, not by editing source code per environment. Bundles are validated by Config Manager and hashed for attestation.
* Rooted or jailbroken detection thresholds and logging verbosity may differ by environment, but admission requirements, OperationContext structure, and capability enforcement remain identical to ensure staging faithfully exercises production rules.

### 13.4 Developer tooling and testing

* SDKs expose helpers for OperationContext assembly, schema validation, pending-write journaling, and admission feedback handling so app developers cannot bypass required behavior accidentally.
...
* Release gates require integration tests against the target backend version, verifying interface compatibility, sync behavior, and OperationContext logging before publication.

## 14. Implementation checklist

1. **App identity**: Slug, `app_id`, manifest, and compatibility matrix registered with App Manager.
2. **OperationContext discipline**: Every request includes immutable OperationContext data derived from authenticated sessions and build metadata, retries rebuild context.
...
4. **Transport compliance**: HTTP or WebSocket clients honor `04-interfaces/**` contracts, enforce TLS or Noise, surface backend admission feedback, and implement structured retries.
...
9. **Security**: Key handling, sandboxing, dependency pinning, tamper detection, and supply-chain checks aligned with `05-security/**`.
...
11. **Integration**: Service-specific behaviors implemented per system service specs, extension manifests respected, and OperationContext hashes verified end to end.
12. **Testing**: Automated tests cover OperationContext assembly, capability gating, schema validation, pending write ordering, sync conflict handling, admission-feedback handling, and error rendering.
