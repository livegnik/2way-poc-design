...
Those files remain normative for log classification, OperationContext semantics, identifier rejection requirements, trace correlation, DoS Guard admission telemetry, and failure handling behavior.
...
* Guaranteeing that identifier and namespace rejection events mandated by `01-protocol/01-identifiers-and-namespaces.md` are recorded as structured logs so Graph Manager and Schema Manager satisfy the protocol logging requirement.
...
* Structured alert signals derived from `security.*` records for DoS Guard Manager consumption. Alert forwarding uses Event Manager bridging where enabled. It does not include raw confidential payloads.
...
Startup fails if mandatory keys are missing or invalid. Reloads follow Config Manager's prepare or commit flow. Log Manager must acknowledge the reload only after verifying new sinks or limits can be applied without data loss.
...
* **Config Manager**: Supplies `log.*` snapshots. Log Manager participates in the prepare and commit reload handshake and emits `operational.config_reload_applied` or `operational.config_reload_rejected` logs.
...
* **Health Manager**: Consumes readiness and liveness data plus sink metrics (queue depth, flush latency). Log Manager must flag readiness false when mandatory sinks or the journal are unavailable.
* **Auth Manager**: Provides OperationContext for HTTP and WebSocket calls. Log Manager relies on this context to tag audit entries.
* **Network Manager and DoS Guard Manager**: Emit security logs for admission decisions per `01-protocol/11-dos-guard-and-client-puzzles.md`. Log Manager ensures those logs are routed to security sinks and optionally to Event Manager while preserving the requirement to log challenge metadata but not raw puzzle payloads. DoS Guard Manager may consume derived alert signals, but Log Manager does not initiate mitigations.
...
* Identifier or namespace violations defined in `01-protocol/01-identifiers-and-namespaces.md` are always captured as `audit.*` or `operational.*` records so that the "record the rejection in the local log" requirement is satisfied regardless of which manager surfaces the error.
...
* Query APIs enforce ACLs: only admin OperationContext identities (as defined in `01-protocol/06-access-control-model.md`) can read security or audit logs. Operational logs may be exposed to broader system operators.
...
* Emitting security or audit payloads through Event Manager or other broadcasts that bypass ACL-enforced query APIs.
