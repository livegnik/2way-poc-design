...
* Owning the admission decision loop for inbound and outbound connections, in accordance with [01-protocol/11-dos-guard-and-client-puzzles.md](../../01-protocol/11-dos-guard-and-client-puzzles.md).
...
* Communicating `allow`, `deny`, and `require_challenge` decisions defined in [01-protocol/11-dos-guard-and-client-puzzles.md](../../01-protocol/11-dos-guard-and-client-puzzles.md) to [Network Manager](10-network-manager.md)'s Bastion Engine without revealing backend implementation details.
* Ensuring `deny` directives cause [Network Manager](10-network-manager.md) to terminate the relevant connection immediately, consistent with Section 8 of [01-protocol/11-dos-guard-and-client-puzzles.md](../../01-protocol/11-dos-guard-and-client-puzzles.md).
...
* Admission decisions (`allow`, `deny`, `require_challenge`) are deterministic given the same telemetry and configuration per [01-protocol/11-dos-guard-and-client-puzzles.md](../../01-protocol/11-dos-guard-and-client-puzzles.md).
* Puzzle difficulty increases monotonically for abusive identities until they return to compliant behavior, matching Section 7.3 of [01-protocol/11-dos-guard-and-client-puzzles.md](../../01-protocol/11-dos-guard-and-client-puzzles.md).
...
[DoS Guard Manager](14-dos-guard-manager.md) enforces a fixed admission lifecycle aligned with [01-protocol/11-dos-guard-and-client-puzzles.md](../../01-protocol/11-dos-guard-and-client-puzzles.md):
...
3. **Decision**: DoS Guard emits `allow`, `deny`, or `require_challenge` along with optional throttle parameters per Section 8 of [01-protocol/11-dos-guard-and-client-puzzles.md](../../01-protocol/11-dos-guard-and-client-puzzles.md).
4. **Puzzle issuance (if required)**: DoS Guard generates challenges defined in Section 7 of [01-protocol/11-dos-guard-and-client-puzzles.md](../../01-protocol/11-dos-guard-and-client-puzzles.md) using [Key Manager](03-key-manager.md)-provided randomness or HMAC keys and returns them to [Network Manager](10-network-manager.md) for transport to the client.
...
* Puzzle responses from [Network Manager](10-network-manager.md) containing `{ challenge_id, solution, connection_id, opaque_payload }` so DoS Guard can replay the validation defined in Section 7.2 of [01-protocol/11-dos-guard-and-client-puzzles.md](../../01-protocol/11-dos-guard-and-client-puzzles.md).
...
* Admission decision objects: `{ connection_id, decision, throttle_params?, challenge_spec? }` returned to [Network Manager](10-network-manager.md)'s Bastion Engine per Sections 6 and 8 of [01-protocol/11-dos-guard-and-client-puzzles.md](../../01-protocol/11-dos-guard-and-client-puzzles.md).
...
Policies are configured via `dos.*` keys and must align with the invariants in [01-protocol/11-dos-guard-and-client-puzzles.md](../../01-protocol/11-dos-guard-and-client-puzzles.md).

## 7. Puzzle generation and verification

Puzzles follow the structure defined in [01-protocol/11-dos-guard-and-client-puzzles.md](../../01-protocol/11-dos-guard-and-client-puzzles.md):
...
* Puzzle validation must confirm `challenge_id`, expiration, `context_binding`, payload fidelity, declared algorithm, and replay status before checking difficulty per Section 7.2 of [01-protocol/11-dos-guard-and-client-puzzles.md](../../01-protocol/11-dos-guard-and-client-puzzles.md).
...
* Escalates to `deny` when puzzle generation fails or telemetry indicates abuse, matching Section 10 of [01-protocol/11-dos-guard-and-client-puzzles.md](../../01-protocol/11-dos-guard-and-client-puzzles.md).
...
* Communicates decisions to [Network Manager](10-network-manager.md) using the interface defined in [01-protocol/11-dos-guard-and-client-puzzles.md](../../01-protocol/11-dos-guard-and-client-puzzles.md).
...
* Register its decision interface with [Network Manager](10-network-manager.md)'s Bastion Engine so all admitted surfaces route through DoS Guard, as mandated by [01-protocol/11-dos-guard-and-client-puzzles.md](../../01-protocol/11-dos-guard-and-client-puzzles.md).
...
* Instruct [Network Manager](10-network-manager.md) to stop admitting new connections on protected surfaces so admission remains fail-closed per [01-protocol/11-dos-guard-and-client-puzzles.md](../../01-protocol/11-dos-guard-and-client-puzzles.md).
...
* If Telemetry Intake fails (queue overflow), DoS Guard logs the event and treats new connections as `require_challenge` until capacity returns, matching Section 10 of [01-protocol/11-dos-guard-and-client-puzzles.md](../../01-protocol/11-dos-guard-and-client-puzzles.md).
...
* Puzzle generation and validation align with [01-protocol/11-dos-guard-and-client-puzzles.md](../../01-protocol/11-dos-guard-and-client-puzzles.md), including challenge structure, validation rules, and expiry handling.
...
* Configuration reloads follow [Config Manager](01-config-manager.md)'s prepare/commit contract and apply atomically per [01-protocol/11-dos-guard-and-client-puzzles.md](../../01-protocol/11-dos-guard-and-client-puzzles.md).
